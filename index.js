...
const UPoints = sequelize.define('user_point',  {
      user_id: { type: Sequelize.STRING }
  , server_id: { type: Sequelize.STRING }
  ,    points: { type: Sequelize.INTEGER, defaultValue: 0 }
...
UPoints
  .sync({ alter: true })
  .then(() => {
    console.log("TABLE CREATED: user_point");
  })
  .catch((err) => {
     console.error("FAILED TABLE CREATE: user_point" + err);
  }) ;

//BIS HIER UND NICHT WEITER!!!!!!!
// get House role from user ???

// Find allHouses & all House Roles
var allHouses                = new Array () ;
var houseRoles               = new Array () ;
Houses.findAll ({where: {server_id:current_server_id}})
  .then ( (houses) => {
    for (let n = 0 ; n < houses.length; n++) {
      var house                = houses [n] ;
      let houseRole            = house.get ().hrole ;
      let houseName            = house.get ({plain: true}).name.toLowerCase () 
      allHouses     [allHouses.length] = houseName ;
      houseRoles    [houseRoles.length]= houseRole ;
      console.log ("--------------------------------------------------------") ;
      console.log (houseName, houseRole) ;
      console.log ("--------------------------------------------------------") ;
...
.then ((users) => {
       for (let x = 0; x < users.length; x++) {
          var user          = users [n]
          let userID        = user.get ({plain:true}).user_id;
          allUsers     [allUsers.length] = userID ;
          console.log ("--------------------------------------------------------") ;
          console.log (userid) ;
          console.log ("--------------------------------------------------------") ;
          //addCommand
...
async function postLeaderboard
  ...
  if (    logChannel
    ...
    let upointRows          = await UPoints.findAll ({ 
                                                             order: [
                                                                      [   'points'
                                                                        , 'DESC'
                                                                      ]
                                                           ...
                                                           , raw: true
                                                           , where: {server_id: current_server_id, points:{[gt]: 0}}
                                                         }
                                                     ) ;
      
    // Create leaderboard houses text
    ...
    for (var i = 0 ; i < 6 ; i++) {
      ...
      var urow                = upointRows [i] ;
      var usubtext            = (i+1)+". "+urow.name.capitalize ()+": "+urow.points+" points" ;
      if (i == 0) {
        subtext              = '**'+subtext+'**' ;
      }
      text                   = "Houses Leaderboard \n" + text +"\n \n" + "Best Users" (text, usubtext).join ('\n') ;
    };
...
// Hier muss ich noch eine Möglichkeit finden, dasselbe für User zu machen... ob das wohl so funktioniert????
  ...
  addCommand ("pointssetup", async function (args) {
    ...
    if (! allUsers.length) {
      args.send ("No user defined in base => use "+process.env.PREFIX+"addhouse <housename> to add houses.") ;
      return ;
    }
    for (var i = 0; i < allUsers.length; i++) {
      Users
        .findOrCreate ( {where: {user_id: allUserss [i], server_id: args.guildId}} )
        .spread ((user, created) => {
          console.log ("FINDORCREATE user_points: " + user.get({plain: true}).name)
          args.send ("Created user entry " + user.get({plain: true}).name + " in upoints table.");
        })
        .catch (err => {
          console.error("FAILED to findOrCreate user entry in user_points " + allUsers [i])
        })
    }
  });
  ...
  addCommand ('pointsreset', async function (args) {
    ...
    for (var i = 0; i < allUsers.length; i++) {
      Users
       .findOne ( {where: {user_id: allUsers [i]} } )
       .then ((user) => {
         user.points          = 0;
         user
           .save ()
           .then (() => {
             console.log (`Reset ${allUsers [i]} points to 0.`) ;
             args.send (`Reset ${allUsers [i]} points to 0.`) ;
           })
           ;
       })
       .catch (err => {
          console.error(`Failed to reset points ${allUsers [i]} points to 0: ` + err);
       })
       ;
    }
  });
//TODO da ist ein Verweis auf postLeaderboard!
  ...
  addCommand ("addhouse", async function(args) {
    ...
    var HouseRole              = args.params [1].substring(3,args.params [1].length-1) ;
    console.log(HouseRole);
    ...
    Houses
      ...
      .spread ( (house, created) => {
        if (created) {
          ...
          //TODO: ON JOINED FÜR USER HINZUFÜGEN?
          allHouses   [allHouses.length] = HouseName ;
  ...
  addCommand ("sethouse", async function(args) {
    ...
    if (allAttr.indexOf (attribute) == -1) {
      args.send ("Invalid attribute {color, icon, desc}") ;
      return ;
    }
    if (! value.length) {
      args.send ("Can't parse empty value") ;
      return ;
    }
    // add in Houses
    Houses
      ...
      Houses
        ...
        .then ((houses) => {
          ...
          for (let n = 0 ; n < houses.length; n++) {
            ...
            let desc           = house.get({plain: true}).desc ;
            embed
...
addCommand("takepoints", async function (args) {
  var user = args.message.member;
  var userMention = "<@!" + args.authorID + ">";
  if (    ! checkPermissions(args, "takePoints")
       && ! checkPermissions(args, "doAllOfTheAbove")
     ) {
    args.send ('You do not have permission to do that.') ;
    return ;
  ...
  if (args.params.length < 2) {
    args.send ("Missing args. Use /takepoints <value> <@someone>") ;
    return ;
  }
  var server_config          = await Configuration.findOne( {where: {server_id: args.guildId}} ) ;
  
  // Check first param is a number
  let args_points           = Number(args.params [0]) ;
  if (      isNaN (args_points)
       && ! Number.isInteger (args_points)
      ) {
    args.send('Point values must be an integer.');
  ...
  if (    args_points < server_config.min_points
    ...
    try {
      // Take points
      // Update DB with points
      //TODO: get the house role from all the users roles!
      var userhouserole = "";
      for (var [key, value] of targetUser.roles){
        let i = 0;
        do {
          if (houseRoles[i] == value.id){
              userhouserole = value.id;
              return;
          }
          i++;
          
        }
        while (userhouserole == "" && i<houseRoles.length) 
          }
        if (userhouserole == ""){
           console.log("There's no house assigned to this user! They can't earn points yet!");
           args.send("There's no house assigned to this user! They can't earn points yet!");
          return;
        }
        
          let userPoints = await UPoints.findOne( {where: {user_id: targetUser.id}});
          userPoints.points = userPoints.points - args_points;
          userPoints.save()
          .then( () => {
          console.log("Took from " + targetUser.name + ": " + args_points + " points" );
          
          } ).catch(err => {
          console.error("Failed take: " + args_points + " points from " + targetUser.name + " " + err);
          args.send("Failed to take " + args_points + " points from " + targetUser.name );
          return;
          });
      
      //let userrole = userPoints.role
      let house = await Houses.findOne( {where: {hrole: userhouserole}} );
      let housename = house.get().name;
      let housePoints = await HPoints.findOne({where:{name: housename.toLowerCase()}});
      housePoints.points = housePoints.points - args_points;
      housePoints.save()
      .then( () => {
        console.log("Took from " + housePoints.name + ": " + args_points + " points" );
      } ).catch(err => {
        console.error("Failed take: " + args_points + " points from " + housename.capitalize() + " " + err);
        args.send("Failed to take " + args_points + " points from " + housename.capitalize() );
        return;
      });
    } catch (e) {
      console.error ("Error on line 470 : ", e) ;
    ...
    var description = "";
    text = targetUserMention + ' lost ' + args_points + ' points for ' + housename.capitalize() + ' from ' + userMention + '.';
    description = [description, 'Lost by ' + targetUserMention + '.'].join(' ');
    if ( args_reason ) {
      text = text + ' *Reason: ' + args_reason + '*';
      description = [description, 'Reason: ' + args_reason].join(' ');
    ...
    var authorName = args_points + ' points from ' + housename.capitalize();
    
    await Houses.findOne ({where:{name:housename.toLowerCase()}})
    .then ( (house) => {
      embed.setAuthor(authorName, house.get().icon).setColor(house.get().color);
    })
    .catch (err => {
...
addCommand("givepoints", async function (args) {
  var user = args.message.member;
  var userMention = "<@!" + args.authorID + ">";
  if (    ! checkPermissions(args, "givePoints")
       && ! checkPermissions(args, "doAllOfTheAbove")
     ) {
    args.send ('You do not have permission to do that.') ;
    return ;
  ...
  if (    args_points < server_config.min_points
    ...
    try {
      ...
      for (var [key, value] of targetUser.roles){
        ...
        if (userhouserole == ""){
           console.log("There's no house assigned to this user! They can't earn or loose points yet!");
           args.send("There's no house assigned to this user! They can't earn or loose points yet!");
          return;
        }
          ...
          console.error("Failed give: " + args_points + " points to " + targetUser.name + " " + err);
          args.send("Failed to give " + args_points + " points to " + targetUser.name );
          return;
          });
      ...
      housePoints.save()
      .then( () => {
        console.log("Gave " + housePoints.name + ": " + args_points + " points" );
      } ).catch(err => {
        console.error("Failed give: " + args_points + " points to " + housename.capitalize() + " " + err);
        args.send("Failed to give " + args_points + " points to " + housename.capitalize() );
        return;
      });
    } catch (e) {
      console.error ("Error on line 470 : ", e) ;
    ...
    var description = "";
    text = targetUserMention + ' earned ' + args_points + ' points for ' + housename.capitalize() + ' from ' + userMention + '.';
    description = [description, 'Earned by ' + targetUserMention + '.'].join(' ');
    if ( args_reason ) {
      text = text + ' *Reason: ' + args_reason + '*';
      description = [description, 'Reason: ' + args_reason].join(' ');
...
client.on ("guildCreate", (guild) => {
  console.log (guild);
  Roles
    .findOrCreate ({where: { permission:"doAllOfTheAbove" , role:"Headmaster", server_id: process.env.GUILD_ID } })
    .spread ((roles, created) => {
